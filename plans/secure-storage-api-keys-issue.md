# Secure Storage Connector API Keys

## Summary

Implement encryption for storage connector API keys (S3/DO Spaces access keys and secrets) that are currently stored in plaintext in the database. This feature adds a master key system using AES-256-GCM encryption to protect sensitive credentials.

## Background

### Current Security Issue

API keys are currently stored in **plaintext** in the SQLite database:

- **Table**: `storage_connectors`
- **Column**: `config` (JSON format)
- **Fields**: `key` (access key) and `secret` (secret key)

**Security Risk:** Anyone with database access can read the plaintext API keys. If the database file is compromised or a SQL injection vulnerability exists, the attacker gains full access to the S3/DO Spaces storage.
...
- [ ] **Create EncryptionHelper** (`src/Phuppi/Helper/EncryptionHelper.php`)
...
- [ ] **Update Bootstrap** (`src/bootstrap.php`)
...
- [ ] **Create Database Migration** (`src/migrations/010_encrypt_storage_keys.php`)
...
- [ ] **Update Storage Loading** (in `src/bootstrap.php` or new method)
...
- [ ] **Add Settings Endpoint** (`src/Phuppi/Controllers/SettingsController.php`)
...
- [ ] **Create Settings UI** (new section in `src/views/settings.latte`)
...
- [ ] **Update SettingsController** (`src/Phuppi/Controllers/SettingsController.php`)
...
- [ ] **Update S3Storage** (`src/Phuppi/Storage/S3Storage.php`)
  - Add logic to resolve keys from env vars if `key_env`/`secret_env` fields exist (future enhancement)

### Phase 5: Documentation

- [ ] **Create Documentation** (`public/docs/secure-storage-keys.md`)
...
| File | Purpose |
|------|---------|
| `src/Phuppi/Helper/EncryptionHelper.php` | Master key generation and encryption utilities |
| `src/migrations/010_encrypt_storage_keys.php` | Migration to encrypt existing keys |
| `public/docs/secure-storage-keys.md` | User documentation |

## Files to Modify

| File | Changes |
|------|---------|
| `src/bootstrap.php` | Initialize EncryptionHelper, check key availability |
| `src/Phuppi/Controllers/SettingsController.php` | Add key generation endpoints |
| `src/views/settings.latte` | Add UI for key management |
